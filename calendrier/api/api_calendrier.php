...
if (isset($_SERVER['REQUEST_METHOD']) && $_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (preg_match("~supprimer-message$~", $_SERVER['REQUEST_URI'], $matches)) {
        ...
        if (isset($donnees['id_message'])) {
            ...
            // Obtenir la liste des tâches de la réunion
            $query = $conn->prepare("DELETE FROM message WHERE id_message = :id");
     ...
     if (preg_match("~obtenir_reunions_groupes$~", $_SERVER['REQUEST_URI'], $matches)) {
        ...
        if (isset($donnees['idGroupe'])) {
            ...
            $query = $conn->prepare("SELECT id_reunions, titre, date, description FROM reunions WHERE id_groupes = :id ORDER BY date");
...
if (isset($_SERVER['REQUEST_METHOD']) && $_SERVER['REQUEST_METHOD'] === 'GET') {
    ...
    if (preg_match("~afficher_groupes$~", $_SERVER['REQUEST_URI'], $matches)) {
        ...
        $query = $conn->prepare("SELECT g.nom, g.id_groupes, g.courriel_enseignant FROM groupes g INNER JOIN utilisateurs_groupes ug ON g.id_groupes = ug.id_groupes WHERE courriel_etudiants = :courriel UNION SELECT nom, id_groupes, courriel_enseignant FROM groupes WHERE courriel_enseignant = :courriel");
        $query->bindParam(":courriel", $_SESSION['courriel'],  PDO::PARAM_STR);
        $query->execute();
        $resultat = $query->fetchAll();
    ...
    if (preg_match("~obtenir_reunions_utilisateur$~", $_SERVER['REQUEST_URI'], $matches)) {
        ...
        $query = $conn->prepare("SELECT r.id_reunions, r.titre, r.description, r.date FROM reunions AS r INNER JOIN utilisateurs_reunions AS ur ON r.id_reunions = ur.id_reunions WHERE courriel_utilisateurs = :courriel ORDER BY r.date, r.heure_debut");
        $query->bindParam(":courriel", $_SESSION['courriel'],  PDO::PARAM_STR);
        $query->execute();
        $resultat = $query->fetchAll();
