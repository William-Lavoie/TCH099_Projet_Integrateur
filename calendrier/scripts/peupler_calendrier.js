$(document).ready(function () {
    ...
    async function chercherReunions(debut, fin, premierJour, dernierJour) {
        try {
            ...
            const reponse = await fetch(
                pathDynamic + "/calendrier/api/api_calendrier.php/chercher_reunions",
                {
                    method: "POST",
            ...
            for (let i = premierJour; i < dernierJour; i++) {
                ...
                for (let j = 0; j < donnees.length; j++) {
                    ...
                    if (
                        dateNombre.toString().padStart(2, "0") ==
                        $("#calendrier").children().eq(i).find("p").text().padStart(2, "0")
                    ) {
                ...
                $("#calendrier")
                    .children()
                    .eq(i)
                    .data("listeReunionsJournee", listeReunionsJournee);
            }
            return donnees;
        } catch (error) {
            throw error;
    ...
    function afficherCalendrier(jour) {
        ...
        const moisDernier = new Date(
            premierDuMois.getFullYear(),
            premierDuMois.getMonth(),
            1 - premierJour
        );
        ...
        for (let i = compteur; i < premierJour + finDuMois; i++) {
            ...
            if (
                i == new Date().getDate() + premierJour - 1 &&
                jour.getFullYear() === new Date().getFullYear() &&
                jour.getMonth() === new Date().getMonth()
            ) {
        ...
        dateFinFormatte = formatterDate(
            new Date(
                premierDuMois.getFullYear(),
                premierDuMois.getMonth(),
                premierDuMois.getDate() + finDuMois - 1
            )
        );
        ...
        dateFinFormatte = formatterDate(
            new Date(
                premierDuMois.getFullYear(),
                premierDuMois.getMonth() + 1,
                premierDuMois.getDate() + 41 - (finDuMois + premierJour)
            )
        );
    ...
    function ouvrirReunion(journee) {
        ...
        for (let i = 0; i < journee.data("listeReunionsJournee").length; i++) {
            ...
            reunion.append(
                "<p>" + journee.data("listeReunionsJournee")[i]["titre"] + "</p>"
            );
            ...
            reunion.append(
                "<p>" +
                    formatterHeure(
                        journee.data("listeReunionsJournee")[i]["heure_debut"]
                    ) +
                    "-" +
                    formatterHeure(journee.data("listeReunionsJournee")[i]["heure_fin"]) +
                    "</p>"
            );

            // Insérer les informations dans la réunion
            reunion.data(
                "listeReunionsJournee",
                journee.data("listeReunionsJournee")[i]
            );
            ...
            $("#panneau-reunions")
                ...
                .on("click", function () {
                    consulterReunion(
                        $(this),
                        journee.data("listeReunionsJournee")[i]["courriel_createur"]
                    );
                });
        }
    ...
    function consulterReunion(reunion, createur) {
        if (!reunion.hasClass("reunion-visible-panneau")) {
            ...
            fetch(
                ...
                .then((data) => {
                    ...
                    for (let i = 0; i < data.length; i++) {
                        let participant = $(
                            "<div class='participant-pour-reunion'><img width='35px' height='35px'><div class='nom-participant-reunion'>" +
                                data[i]["nom"] +
                                "</div></div>"
                        );
            ...
            reunion.append(
                "<div id='boutons-pour-panneau'><div id='btn-panneau-reunion'></div></div>"
            );

            // Créer des écouteurs d'évènements pour les boutons
            let boutonJoindre = $(
                "<button id='consulter-reunion-panneau'>Joindre</button>"
            );
            ...
            fetch(
                ...
                .then((reponse) => {
                    if (reponse == createur) {
                        let boutonModifier = $(
                            "<button id='modifier-reunion-panneau'>Modifier</button>"
                        );
                        ...
                        boutonEffacer.on("click", function () {
                            if (
                                confirm(
                                    "La supression d'une réunion est irréversible, êtes-vous sûr de vouloir continuer?"
                                )
                            ) {
    ...
    function joindreReunion(reunion) {
        ...
        window.location.href =
            "../../réunions/réunions.html?info=" + encodeURIComponent(idReunion);
    }
    ...
    function modifierReunion(reunion) {
        ...
        localStorage.setItem(
            "reunionIdentifiant",
            reunion.data("listeReunionsJournee")["id_reunions"]
        );
        ...
        $("#debut-reunion").val(
            formatterHeure(reunion.data("listeReunionsJournee")["heure_debut"])
        );
        $("#fin-reunion").val(
            formatterHeure(reunion.data("listeReunionsJournee")["heure_fin"])
        );
        $("#date-reunion").val(reunion.data("listeReunionsJournee")["date"]);
        $("#description-reunion").val(
            reunion.data("listeReunionsJournee")["description"]
        );
        ...
        if (reunion.data("listeReunionsJournee")["id_groupes"] == null) {
            ...
            fetch(
                ...
                .then((data) => {
                    ...
                    for (let i = 0; i < data.length; i++) {
                        if (
                            reunion.data("listeReunionsJournee")["courriel_createur"] !=
                            data[i]["courriel_utilisateurs"]
                        ) {
                            // Contenant pour le participant
                            const nouveauParticipant = $(
                                "<div class='nom-participant'> <p></p> </div> "
                            );
                            const boutonSupprimer = $(
                                "<button class='supprimer-participant'>🗑</button>"
                            );
                            ...
                            nouveauParticipant
                                .children("p")
                                .text(data[i]["courriel_utilisateurs"]);
        ...
        fetch(
            ...
            .then((data) => {
                for (let i = 0; i < data.length; i++) {
                    ...
                    const boutonSupprimer = $(
                        "<button class='supprimer-tache'>🗑</button>"
                    );
    ...
    function supprimerReunion(reunion) {
        ...
        fetch(
            pathDynamic + "/calendrier/api/api_calendrier.php/supprimer_reunion",
            {
                method: "POST",
                headers: { "Content-Type": "application/json" },
                body: JSON.stringify(donnees),
    ...
    $("#btn-dernier-mois").on("click", function () {
        jourCourant = new Date(
            jourCourant.getFullYear(),
            jourCourant.getMonth() - 1
        );
        afficherCalendrier(jourCourant);
    });
